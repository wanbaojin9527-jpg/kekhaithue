...
const App: React.FC = () => {
  ...
  /**
   * CẤU HÌNH QUAN TRỌNG: 
   * Luôn khởi tạo từ MASTER_CONFIG (config.ts) để đảm bảo sau khi deploy
   * nội dung mới nhất sẽ được hiển thị ngay cho người dùng.
   */
  const [contractData, setContractData] = useState<ContractData>(MASTER_CONFIG);

  // Hiệu ứng đồng bộ hóa khi MASTER_CONFIG thay đổi (khi deploy bản mới)
  useEffect(() => {
  ...
  const handleManualSave = () => {
    // Lưu tạm vào localStorage chỉ để xem thử trên máy hiện tại
  ...
  const copyToClipboard = () => {
    ...
    alert("Đã sao chép nội dung cấu hình! Hãy dán vào file config.ts trên máy tính của bạn và deploy lại.");
  };
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col items-center py-8 px-4 font-arial">
      {/* Nút Admin Panel để hỗ trợ lấy JSON nhanh */}
      <button 
        onClick={handleAdminClick}
        className={`fixed bottom-6 right-6 z-50 p-4 rounded-full shadow-2xl transition-all no-print flex items-center gap-2 font-bold text-sm ${isEditMode ? 'bg-red-500 text-white' : 'bg-indigo-900 text-white hover:scale-105 opacity-50 hover:opacity-100'}`}
      >
        {isEditMode ? <X size={20} /> : <Settings2 size={20} />}
        <span>{isEditMode ? 'Đóng' : 'Sửa Nhanh'}</span>
      </button>
      ...
      {isEditMode && (
        <div className="fixed inset-y-0 left-0 w-full md:w-[450px] bg-white shadow-2xl z-40 p-0 overflow-hidden no-print border-r border-slate-200 flex flex-col animate-in slide-in-from-left duration-300">
          ...
          <div className="p-6 bg-slate-50 border-t border-slate-200 space-y-3">
            <button onClick={handleManualSave} className={`w-full py-4 text-white rounded-xl font-bold flex items-center justify-center gap-2 transition-all ${saveStatus ? 'bg-green-600' : 'bg-indigo-900'}`}>{saveStatus ? <CheckSquare size={18} /> : <Save size={18} />} {saveStatus ? 'ĐÃ LƯU XEM THỬ' : 'LƯU TẠM THỜI'}</button>
            <button onClick={copyToClipboard} className="w-full py-3 bg-white text-indigo-700 border border-indigo-200 rounded-xl font-bold text-xs flex items-center justify-center gap-2 hover:bg-indigo-50 transition-colors uppercase tracking-widest"><Code size={14} /> COPY CONFIG JSON ĐỂ LƯU VĨNH VIỄN</button>
